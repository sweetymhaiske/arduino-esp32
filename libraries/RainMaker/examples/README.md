...
### RMaker.initNode()
This API initializes the ESP RainMaker Agent, creates the node and initializes the WiFi.
```
...
### RMaker.start()
It starts the ESP RainMaker Agent. If the ESP32 is already provisioned, it will join the configured Wi-Fi network. Else, it will start Wi-Fi provisioning. 
```
RMaker.start()
```
...
It removes device from to the node.
```
RMaker.removeNodeDevice(const node_t node, RMakerGenericClass device);
```
* **Parameters Required**
1. `node` : Node handle created using RMaker.initNode()
2. `device` : Device object
...
### addCb()
It registers read and write callback for the device that will be invoked as per requests received from the cloud (or other paths as may be added in future).
```
...
> NOTE : If private data is not specified then standard device will not create.
...
```
my_param.createService(const char *serv_name, const char *serv_type, void *priv_data);
```
* **Parameters Required**
...
> NOTE : This service should be added to the node using `RMaker.addNodeDevice(node, my_service);`
...
OTAService(NULL);
```
`NULL` : private data for the service should be specified, which will be used in callback.
> NOTE : If private data is not specified then standard device will not create.
## NOTE
The care should be taken that the RMaker.initNode() should be the first API call and RMaker.start().
